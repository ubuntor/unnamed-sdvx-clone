...
public:
	...
	virtual bool AsyncLoad() override
	{
		...
		m_audioOffset = m_songOffset + g_gameConfig.GetInt(GameConfigKeys::AudioOffset);
...
#define NVG_FLUSH() do { \
	...
	{
		return m_audioOffset + m_songOffset + m_tempOffset;
	}
	...
	virtual void RenderDebugHUD(float deltaTime)
	{
		...
		textPos.y += RenderText(Utility::Sprintf("Offset (ms): Visual %d, Song %d, Audio %d (%d)",
			m_playback.visualOffset, m_songOffset, GetAudioOffset(), g_audio->audioLatency), textPos).y;
