...
PracticeModeSettingsDialog::Tab PracticeModeSettingsDialog::m_CreateGameSettingTab()
{
    ...
    Setting visualOffsetSetting = CreateIntSetting(GameConfigKeys::VisualOffset, "Visual offset", { -200, 200 });
    visualOffsetSetting->setter.Clear();
    visualOffsetSetting->setter.AddLambda([this](const SettingData& data) {
        g_gameConfig.Set(GameConfigKeys::VisualOffset, data.intSetting.val);
        onSettingChange.Call();
    });
    gameSettingTab->settings.emplace_back(std::move(visualOffsetSetting));

    Setting audioOffsetSetting = CreateIntSetting(GameConfigKeys::AudioOffset, "Audio offset", { -200, 200 });
    audioOffsetSetting->setter.Clear();
    audioOffsetSetting->setter.AddLambda([this](const SettingData& data) {
        g_gameConfig.Set(GameConfigKeys::AudioOffset, data.intSetting.val);
        onSettingChange.Call();
    });
    gameSettingTab->settings.emplace_back(std::move(audioOffsetSetting));
